...
class KANLayer(nn.Module):
    def __init__(self, in_dim, out_dim, num_centers=5, sigma=0.1, device='cpu'):
        super().__init__()
        self.in_dim = in_dim
        self.out_dim = out_dim
        ...
        # Create Gaussian centers and basis
        self.centers = gaussian_grid(num_centers).to(device)
        self.basis = GaussianBasis(self.centers, sigma)
        
        # Initialize coefficients for linear combination of basis functions
        self.coef = nn.Parameter(torch.randn(out_dim, in_dim, num_centers))
        
    def forward(self, x):
        # Compute basis function values
        basis_vals = self.basis(x)  # (batch_size, num_centers)
        
        # Compute linear combination
        out = torch.einsum('bk,oik->boi', basis_vals, self.coef)
        return out
