...
class GaussianBasis:
    def __init__(self, centers, sigma=0.1):
        ...
        Args:
            centers: tensor of center points for Gaussian functions
    ...
    def __call__(self, x):
        """
        Compute Gaussian basis function values at points x
        Args:
            x: input points (batch_size, 1)
        Returns:
            basis function values (batch_size, n_centers)
        """
        # Reshape x and centers for broadcasting
        x = x.unsqueeze(-1)  # (batch_size, 1, 1)
        centers = self.centers.reshape(1, -1)  # (1, n_centers)
        
        # Compute Gaussian values
        diff = (x - centers) / self.sigma
        return torch.exp(-0.5 * diff**2)

def gaussian_grid(num_centers, range_min=-1, range_max=1):
    """
    Create evenly spaced centers for Gaussian basis functions
    """
    return torch.linspace(range_min, range_max, num_centers)
